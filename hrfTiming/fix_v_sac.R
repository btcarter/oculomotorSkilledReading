...
setwd("~/Documents/Research/fMRI_data/Reading/Compute_data/TimingFiles/saccade")
...
group <- read.csv("~/Documents/Research/fMRI_data/Reading/Compute_data/TimingFiles/source_csvs/fixvsac.csv")
colnames(group)
...
for (i in unique(mdata$RECORDING_SESSION_LABEL)) {
  ...
  if (nrow(sub1data) > 0) {
    ...
    write.table(sub1data, paste("sac/",i, "_sac", ".txt", sep = ""), sep = "\t", na = "", col.names = FALSE, row.names = FALSE, quote = FALSE)
    }
}
...
for (i in unique(mdata$RECORDING_SESSION_LABEL)) {
  ...
  if (nrow(sub1data) > 0) {
    ...
    sub1data = dcast(sub1data, RUN ~ variable)
    sub1data = sub1data[2:ncol(sub1data)]
    write.table(sub1data, paste("fix/",i, "_fix", ".txt", sep = ""), sep = "\t", na = "", col.names = FALSE, row.names = FALSE, quote = FALSE)
  }
}
