...
setwd("~/Documents/Research/fMRI_data/Reading/Compute_data/TimingFiles/oculomotor")
...
for (i in unique(mdata$RECORDING_SESSION_LABEL)) {
  ...
  if (nrow(sub1data) > 0) {
    ...
    write.table(sub1data, paste("fixations/",i,".txt", sep = ""), sep = "\t", na = "", col.names = FALSE, row.names = FALSE, quote = FALSE)
  }
}
